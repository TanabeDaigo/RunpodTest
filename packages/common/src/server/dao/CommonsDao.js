...
const log = new logjs("CommonsDao");

/**
 * 共通マスタ情報を管理するDAOクラス
 * 共通マスタの検索、取得などのデータベース操作を担当
 */
@injectable()
class CommonsDao extends AbstractDao {
  constructor() {
    super("commons");
  }
   ...
   * @param {string} [params.sortKey="type"] - ソートキー
  ...
  async find(params) {
    ...
    const {
      page = 1,
      pageSize = 100,
      sortKey = "type", // 指定していなければプライマリキーの先頭
