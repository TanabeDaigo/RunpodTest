...
const log = new logjs("SrcTemplatesDao");

/**
 * ソーステンプレート情報を管理するDAOクラス
 * ソーステンプレートの検索、取得などのデータベース操作を担当
 */
@injectable()
class SrcTemplatesDao extends AbstractDao {
  constructor() {
    super("src_templates");
  }
   ...
   * @param {string} [params.sortKey="template_id"] - ソートキー
  ...
  async find(params) {
    ...
    const {
      page = 1,
      pageSize = 100,
      sortKey = "template_id", // 指定していなければプライマリキーの先頭
    ...
    try {
      ...
      // 取得するカラムの指定
      _sqljs.select([
        "sr.template_id as template_id",
        "sr.type as type",
        "sr.name as name",
        "sr.is_deleted as is_deleted",
        "sr.dbcount as dbcount",
        "sr.regist_user as regist_user",
        "sr.update_user as update_user",

        "DATE_FORMAT(sr.created_at, '%Y/%m/%d %H:%i') as created_at", // 登録日時をフォーマット
        "DATE_FORMAT(sr.updated_at, '%Y/%m/%d %H:%i') as updated_at", // 更新日時をフォーマット
      ]);

      // テーブル結合の設定
      _sqljs.from(["src_templates sr"]);

      // 基本条件：有効なレコードのみ
      _sqljs.where("sr.is_deleted = false");

      // オプション条件の追加
      log.debug("auto_where", params);
      _sqljs.autoWhere("sr", params);
...
export default SrcTemplatesDao;
