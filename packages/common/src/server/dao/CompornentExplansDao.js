...
const log = new logjs("CompornentExplansDao");

/**
 * コンポーネント説明情報を管理するDAOクラス
 * コンポーネント説明の検索、取得などのデータベース操作を担当
 */
@injectable()
class CompornentExplansDao extends AbstractDao {
  constructor() {
    super("compornent_explans");
  }
  ...
  async find(params) {
    ...
    try {
      ...
      _sqljs.select([
        ...
        "co.explan as explan",
        "co.status as status",
        "co.dbcount as dbcount",
        "co.registuser as registuser",
        "co.updateuser as updateuser",

        "DATE_FORMAT(co.created_at, '%Y/%m/%d %H:%i') as created_at", // 登録日時をフォーマット
        "DATE_FORMAT(co.updated_at, '%Y/%m/%d %H:%i') as updated_at", // 更新日時をフォーマット
      ]);

      // テーブル結合の設定
      _sqljs.from(["compornent_explans co"]);

      // 基本条件：有効なレコードのみ
      _sqljs.where("co.status != 1");

      // オプション条件の追加
      log.debug("auto_where", params);
      _sqljs.autoWhere("co", params);
...
export default CompornentExplansDao;
