...
const log = new logjs("SettingsDao");

/**
 * 設定情報を管理するDAOクラス
 * 設定の検索、取得などのデータベース操作を担当
 */
@injectable()
class SettingsDao extends AbstractDao {
   ...
   * @param {string} [params.sortKey="setting_id"] - ソートキー
  ...
  async find(params) {
    ...
    const {
      page = 1,
      pageSize = 100,
      sortKey = "setting_id", // 指定していなければプライマリキーの先頭
    ...
    try {
      ...
      // 取得するカラムの指定
      _sqljs.select([
        "se.setting_id as setting_id",
        "se.type as type",
        "se.contexts as contexts",
        "se.is_deleted as is_deleted",
        "se.dbcount as dbcount",
        "se.regist_user as regist_user",
        "se.update_user as update_user",

        "DATE_FORMAT(se.created_at, '%Y/%m/%d %H:%i') as created_at", // 登録日時をフォーマット
        "DATE_FORMAT(se.updated_at, '%Y/%m/%d %H:%i') as updated_at", // 更新日時をフォーマット
      ]);

      // テーブル結合の設定
      _sqljs.from(["settings se"]);

      // 基本条件：有効なレコードのみ
      _sqljs.where("se.is_deleted = false");

      // オプション条件の追加
      log.debug("auto_where", params);
      _sqljs.autoWhere("se", params);
