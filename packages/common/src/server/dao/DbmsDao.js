...
const log = new logjs("DbmsDao");

/**
 * データベース管理システム情報を管理するDAOクラス
 * データベース管理システムの検索、取得などのデータベース操作を担当
 */
@injectable()
class DbmsDao extends AbstractDao {
  constructor() {
    super("dbms");
  }

  /**
   * データベース管理システム情報を検索・取得する
   * @param {Object} params - 検索パラメータ
   * @param {number} [params.page=1] - ページ番号
   * @param {number} [params.pageSize=100] - 1ページあたりの件数
   * @param {string} [params.sortKey="dbms_id"] - ソートキー
  ...
  async find(params) {
    ...
    const {
      page = 1,
      pageSize = 100,
      sortKey = "dbms_id", // 指定していなければプライマリキーの先頭
    ...
    try {
      ...
      // 取得するカラムの指定
      _sqljs.select([
        "db.dbms_id as dbms_id",
        "db.dbms_name as dbms_name",
        "db.sort as sort",
        "db.is_deleted as is_deleted",
        "db.status as status",
        "db.regist_user as regist_user",
        "db.update_user as update_user",

        "DATE_FORMAT(db.created_at, '%Y/%m/%d %H:%i') as created_at", // 登録日時をフォーマット
        "DATE_FORMAT(db.updated_at, '%Y/%m/%d %H:%i') as updated_at", // 更新日時をフォーマット
      ]);
      ...
      // 基本条件：有効なレコードのみ
      _sqljs.where("db.is_deleted = false");

      // オプション条件の追加
      log.debug("auto_where", params);
      _sqljs.autoWhere("db", params);
