/**
 * @file generate_CheckBoxEx.js
 * @description データベーススキーマからuseCheckBoxEx.jsを自動生成するスクリプト
 *
 * このスクリプトは、db_schema.jsのテーブルカラム情報を参照して、
 * tinyint、boolean型のカラムを抽出し、useCheckBoxEx.jsを自動生成します。
 *
 * @author MetroJS Team
 * @version 1.0.0
 * @since 2024-12-19
...
// 対象のデータ型
const TARGET_TYPES = ["tinyint", "boolean"];
...
const generateFunctionName = (columnName) => {
  return `checkbox_${columnName}`;
};

/**
 * チェックボックス関数のJSDocコメントを生成
 * @param {string} columnName - カラム名
...
const generateJSDoc = (columnName, comment) => {
   ...
   * @returns {JSX.Element} ${comment}チェックボックスのJSX要素
   */`;
};
...
const generateFunction = (columnName, columnInfo) => {
  ...
  const ${functionName} = (params = {}, is_debug = false) => {
    ...
    return formProps.checkbox("${columnName}", { label: "${columnInfo.comment}", is_standard, ...params }, is_debug);
  };`;
};
...
const main = async () => {
  try {
    ...
    // useCheckBoxEx.jsの出力パス
    const outputPath = outputPathArg ? path.resolve(outputPathArg) : path.join(__dirname, "../../src/client/hooks/FormEx/useCheckBoxEx.js");

    // ファイルがすでに存在するかチェック
    if (fs.existsSync(outputPath) && !force) {
      console.log(`ℹ️  useCheckBoxEx.jsはすでに存在しています: ${outputPath}`);
    ...
    if (fs.existsSync(outputPath) && force) {
      console.log(` 既存のuseCheckBoxEx.jsを上書きします: ${outputPath}`);
    }
    ...
    // useCheckBoxEx.jsのテンプレート
    const useCheckBoxExTemplate = `/**
...
const useCheckBoxEx = (form = {}, formProps = {}) => {
  const is_standard = false; // 標準のチェックボックスを使用するかどうか
${functions}
...
export default useCheckBoxEx;`;
    ...
    const fileSizeKB = Math.round(Buffer.byteLength(useCheckBoxExTemplate, "utf8") / 1024);

    console.log(`ファイル生成中... (${fileSizeKB}KB)`);
    fs.writeFileSync(outputPath, useCheckBoxExTemplate, "utf8");

    // ファイル生成完了を待機（CursorのUI更新のため）
    await new Promise((resolve) => setTimeout(resolve, 100));

    console.log(`✅ useCheckBoxEx.jsが正常に生成されました`);
