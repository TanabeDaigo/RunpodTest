 ...
 * - カスタムダイアログの統合
 *
 * @returns {JSX.Element} ユーザー管理ページのレンダリング結果
 */
"use client";
...
export default function Page() {
  ...
  const [form, formProps] = useUsers(initState, state, actions);
  ...
  const onShowTableRow = (dataKey, rowData) => {
    if (dataKey === "_edit") {
      return (
        <button
          onClick={() => {
            dialogProps.open({ user_id: rowData.user_id, is_new: false, title: "更新" }, (result) => {
              if (result?.result) {
                formProps.find();
              }
            });
          ...
          <Paper sx={{ p: 2, width: "100%", minWidth: "100%" }}>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} md={3}>
                {formProps.input_user_id()} {/* ユーザーID */}
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
                {formProps.input_last_name()} {/* 姓 */}
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
                {formProps.input_user_name()} {/* ユーザー名 */}
              </Grid>
            </Grid>
