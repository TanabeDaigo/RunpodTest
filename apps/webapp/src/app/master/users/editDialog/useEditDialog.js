...
export const useEditDialog = (state, actions, params) => {
  ...
  // パラメータからユーザーIDと新規作成フラグを取得
  log.debug("useEditDialog params", params);

  const initForm = {
    auth: 1, // デフォルトの権限
    status: 0, // デフォルトのステータス
    login_id: "", // ログインID（空文字）
    last_name: "", // 姓（空文字）
    user_name: "", // ユーザー名（空文字）
  };
  ...
  const get_one = async () => {
    ...
    log.debug(`get_one user_id:${user_id}`);
    // ローディング表示を開始
    actions.showLoading();
    try {
      // APIにユーザー取得リクエストを送信
      const res = await api.post({ mode: "get_one", user_id: user_id });
  ...
  const save = async () => {
    ...
    actions.showLoading();
    try {
      log.debug(`save user_id:${user_id} is_new:${is_new}`, params);
      // APIに送信するパラメータを構築
      const _params = {
  ...
  const delete_one = async () => {
    ...
    try {
      const user_id = params?.user_id || null;
      // APIに削除リクエストを送信
      const res = await api.post({ mode: "delete", user_id: user_id });
      return res;
    } catch (error) {
