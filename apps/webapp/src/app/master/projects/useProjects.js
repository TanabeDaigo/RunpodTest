...
const log = new logjs("useProjects");
 ...
 * @returns {[Object, Object]} フォーム状態とフォームプロパティを含むオブジェクトの配列
 */
export const useProjects = (initState, state, actions) => {
  // フォーム状態の管理
  const [form, formProps] = hooks.useFormEx(initState, state, actions);

  // ソート情報の状態管理（デフォルト: project_idの降順）
  const [sortInfo, setSortInfo] = useState({ sortKey: "project_id", sortOrder: "desc" });
  ...
  async function find(page = 1, pageSize = 10, formData = null, _sortInfo = null) {
    log.debug("useProjects find", page, pageSize, formData, _sortInfo);
    ...
    try {
      ...
      log.debug("useProjects find res", res);
      ...
      log.error("useProjects find error", error);
  ...
  const makeGrid = (params = {}) => {
    ...
    return components.VirtualizedTable({
      ...
      data: projects, // 表示するデータ配列
      columns: projectColumns, // 列の定義
      onHeaderClick,
      onShowTableRow,
    });
  };
